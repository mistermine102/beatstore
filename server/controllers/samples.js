  ...
  export const uploadSample = async (req, res) => {
    ...
    await updateUploadsOnCreate(req.userId, newSample.type, newSample.constructor.modelName, newSample._id)

    //send response
    res.json({ newTrack: newSample })
  }

  export const deleteSample = async (req, res) => {
    const { trackId } = req.body

    const sample = await Sample.findById(trackId)
    ...
    await Like.deleteMany({ trackType: 'sample', trackId })
