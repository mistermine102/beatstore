      ...
      <div class="flex flex-wrap mb-2">
        <div v-for="filter in filters">
          <p v-if="filter.type === 'exact' && filter.value" class="mr-4">
            <span class="text-textLightGrey/90">{{ filter.label }}:</span> <span class="active-filter-badge rounded-lg" @click="removeFilter(filter.id)">{{ filter.value }}</span>
          </p>
          <p v-if="filter.type === 'range' && (filter.value.min || filter.value.max)" class="mr-4">
            <span class="text-textLightGrey/90">{{ filter.label }}:</span>
            <span class="active-filter-badge rounded-lg" @click="removeFilter(filter.id)"
          ...
          <p v-if="filter.type === 'set' && filter.values.filter(el => el[1]).length" class="flex flex-wrap mr-4">
            ...
            <div v-for="val in filter.values">
             <span v-if="val[1]" :key="val[0]" @click="removeFilter(filter.id, val[0])" class="active-filter-badge rounded-lg mr-1">{{ val[0] }}</span>
            </div>
          </p>
        </div>
      </div>
